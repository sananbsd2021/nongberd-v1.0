...
export async function GET(request: BunRequest): Promise<Response> {
  const { data: edunews, error } = await supabase
    .from<EduNewsItem>('edunews')
    .select('*')
  ...
  return new Response(JSON.stringify({ success: true, data: edunews, message: 'Data items fetched successfully.' }), {
    status: 200,
    headers: { 'Content-Type': 'application/json' },
  });
}
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    const { data, error } = await supabase.from('edunews').insert([body]).select();
